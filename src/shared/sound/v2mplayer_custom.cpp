...
/**/
void V2MPlayer::Init(sU32 a_tickspersec) 
{
    m_tpc=a_tickspersec; m_base.valid=0; 
    m_synth = (sU8*)nlMalloc( synthGetSize() );
}
/**/
...
/**/
unsigned int V2MPlayer::soundLen()const
{
    return m_base.maxtime;
}
/**/
unsigned int V2MPlayer::playPos()
{
	/* ‘å•Ï‰ö‚µ‚¢‚Ì‚Å—˜—p‚µ‚È‚¢‚±‚Æ‚ð„§ */
    int pos = m_state.time;
    return pos;
}
...
/////////////////////////////////////////////
{
	...
	this32=(td*8/m_base.timediv);
	posnum+=this32+1;
	dp= (sS32*)nlMalloc( sizeof(sS32)*2*posnum );
	gnr=0;
	gp=m_base.gptr;
	lastevtime=0;
	pb32=32;
