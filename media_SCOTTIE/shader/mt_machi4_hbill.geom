...
{	
	PSIn output = (PSIn)0;
	const f4 col[] = {f4(1,1,1,1), f4(0.8, 0.1, 0.1, 0.8*max(0,1.1-sceneTime.x))};
	...
	f3 st = max(0,1 - 0.2);
	f3 rst = max(0,sceneTime.x - 0.2);
	Rotate(rrr * (rst + 0.3), rotmat);
	f3 offset = f3(dir.x,10 * abs(op.z)*sin(op.z/2),dir.z) * st * st * 3;
	...
	if (input[0].C.x > 0)
	{
		for( uint i=0; i<3; i++ )
		{
			...
			output.P.xyz += ct + offset - f3(0,sceneTime.x * 100,0);
			// output.P = input[i].P + f4(dir * sceneTime.x,0);
			output.N = input[i].N;
			float sy = 2*cos(ct.y/20.0);
